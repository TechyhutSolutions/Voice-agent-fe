...
// Image metadata
export const alt = 'Techyhut Labs Voice Agent';
export const size = {
  width: 1200,
  height: 628,
};
...
export default async function Image() {
  ...
  const usesDefaultBranding =
    logoUri === APP_CONFIG_DEFAULTS.logoDark || logoUri === APP_CONFIG_DEFAULTS.logo;
  const defaultWordmarkUri = 'public/techyhut-wordmark.svg';
  const wordmarkUri = usesDefaultBranding ? defaultWordmarkUri : logoUri;
  ...
  // wordmark
  const { base64: wordmarkSrcBase64, dimensions: wordmarkDimensions } = await getImageData(
    wordmarkUri,
    defaultWordmarkUri
  );
  const wordmarkSize = scaleImageSize(wordmarkDimensions, usesDefaultBranding ? 32 : 64);

  // logo
  const { base64: logoSrcBase64, dimensions: logoDimensions } = await getImageData(
    logoUri,
    'public/techyhut-logo-dark.svg'
  );
